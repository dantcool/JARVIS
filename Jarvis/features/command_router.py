...
class CommandRouter:
    def __init__(
        ...
        self.local_mode_blocked = local_mode_blocked
        ...
        self.calendar_phrases = calendar_phrases
        ...
        self.handlers = [
            ...
            (lambda c: re.search('weather', c), self.handle_weather),
            (lambda c: re.search('tell me about', c), self.handle_wiki),
            (lambda c: "buzzing" in c or "news" in c or "headlines" in c, self.handle_news),
            (lambda c: 'search google for' in c, self.handle_google_search),
            (lambda c: "play music" in c or "hit some music" in c, self.handle_play_music),
            (lambda c: 'youtube' in c, self.handle_youtube),
            (lambda c: "email" in c or "send email" in c, self.handle_email),
            ...
            (lambda c: any(phrase in c for phrase in self.calendar_phrases), self.handle_calendar),
    ...
    def handle_weather(self, command):
        ...
        if self.local_only_mode:
            weather_res = (
                f"Real time weather for {city} is disabled in local only mode. "
                "Please disable local_only_mode to use weather API."
            )
        else:
            weather_res = self.assistant.weather(city=city)
        print(weather_res)
    ...
    def handle_news(self, command):
        if self.local_only_mode:
            self.local_mode_blocked("News")
            return
        news_res = self.assistant.news()
        if not news_res:
            self.speak("Sorry sir, I could not fetch headlines right now")
            return
    ...
    def handle_google_search(self, command):
        self.assistant.search_anything_google(command)
    ...
    def handle_email(self, command):
        if self.local_only_mode:
            self.local_mode_blocked("Email")
            return
        sender_email = self.config.email
        sender_password = self.config.email_password
        try:
            self.speak("Whom do you want to email sir ?")
            recipient = self.assistant.mic_input()
            receiver_email = self.email_dict.get(recipient)
            if receiver_email:
                self.speak("What is the subject sir ?")
                subject = self.assistant.mic_input()
                self.speak("What should I say?")
                message = self.assistant.mic_input()
                msg = 'Subject: {}\n\n{}'.format(subject, message)
                self.assistant.send_mail(sender_email, sender_password, receiver_email, msg)
                self.speak("Email has been successfully sent")
                time.sleep(2)
                return
            self.speak("I coudn't find the requested person's email in my database. Please try again with a different name")
        except Exception:
    ...
    def handle_calendar(self, command):
        if self.local_only_mode:
            self.local_mode_blocked("Google calendar")
            return
        self.assistant.google_calendar_events(command)
